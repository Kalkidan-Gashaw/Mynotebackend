...
app.post("/add-note", authenticateToken, async (req, res) => {
  const { title, content, tags, reminderTime } = req.body; // Include reminderTime
  const user = req.user;
  ...
  try {
    const note = new Note({
      ...
      tags: tags || [],
      userId: user.id,
      reminderTime, // Save reminderTime
      reminderTriggered: false, // Initialize reminderTriggered
    });
...
app.put("/edit-note/:noteId", authenticateToken, async (req, res) => {
  ...
  if (!title && !content && !tags && reminderTime === undefined) {
    return res
      .status(400)
  ...
  try {
    ...
    if (reminderTime !== undefined) {
      note.reminderTime = reminderTime; // Update reminderTime
      note.reminderTriggered = false; // Reset reminderTriggered when reminderTime is changed
    }
...
app.listen(5560, () => {
  console.log("Server running on port 5560");
});
